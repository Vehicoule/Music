...
fn init_schema(connection: &Connection) -> Result<(), CoreError> {
    ...
    ensure_source_index_schema(connection).map(|_| ())
}
...
pub fn clear_source_index(path: impl AsRef<Path>) -> Result<(), CoreError> {
    ...
    connection
        ...
        .map_err(sql_error)?;
    sync_source_index_version(&connection, false).map(|_| ())
}
...
fn ensure_source_index_schema(
    ...
    let status = sync_source_index_version(connection, rebuilt)?;
    if status.rebuilt {
        connection
            .execute("DELETE FROM source_index", [])
            .map_err(sql_error)?;
