...
fn iso8601_to_timestamp(value: &str) -> Result<i64, CoreError> {
    ...
    parse_rfc3339_timestamp(value)
        .ok_or_else(|| CoreError::new("invalid_timestamp", format!("invalid timestamp: {value}")))
}

fn parse_rfc3339_timestamp(value: &str) -> Option<i64> {
    let separator = value.find(['T', 't'])?;
    let date = &value[..separator];
    let time_with_zone = &value[separator + 1..];
    ...
    let (time, offset_seconds) = if let Some(time) = time_with_zone.strip_suffix(['Z', 'z']) {
        (time, 0)
    } else {
    ...
    let second_text = second_part.split('.').next()?;
